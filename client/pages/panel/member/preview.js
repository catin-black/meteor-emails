...
Template.preview.helpers({
    member: function() {
        ...
        member.createdAt = member.createdAt ? moment(member.createdAt).format('YYYY MMMM DD, HH:MM:SS') : null;
        member.updatedAt = member.createdAt ? moment(member.createdAt).format('YYYY MMMM DD, HH:MM:SS') : null;
        member.sentAt = member.sentAt ? moment(member.sentAt).format('YYYY MMMM DD, HH:MM:SS') : null;

        if (member.categories && member.categories.length) {
            let categ = ""
            member.categories.forEach(function(cat) {
                return categ = categ + '<span class="badge badge-info px-2 mx-1">' + cat + '</span>';
            });
        ...
        if (member.tags && member.tags.length) {
            ...
            member.tags.forEach(function(cat) {
                tags = tags + '<span class="badge badge-info px-2 mx-1">' + cat + '</span>';
            });
...
Template.preview.events({
    ...
    'click button.block': function(event, template) {
        const button = $(event.target);
        const value = button.attr('data-block') && button.attr('data-block') == "true" ? true : false;
        callServerMethod({
            methodName: "block",
            params: {
                block: value,
