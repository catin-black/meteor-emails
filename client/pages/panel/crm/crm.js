...
Template.pagePanelCrm.events({
    ...
    'click #deleteSelected': function(event, template) {
        if (!template.selectedMembers.get().length) showWarningAlert(TAPi18n.__('alerts.warning'), TAPi18n.__('alerts.noSelected'));
        else {
            showConfirmDialog({
    ...
    'click #sendToSelected': function(event, template) {
        if (!template.selectedMembers.get() || !template.selectedMembers.get().length) showWarningAlert(TAPi18n.__('alerts.limitError'), TAPi18n.__('alerts.limitError2'));
        else if (template.howManyLeft.get() < 1) showWarningAlert(TAPi18n.__('alerts.warning'), TAPi18n.__('alerts.noSelected'));
        else if (template.howManyLeft.get() < template.selectedMembers.get().length) showWarningAlert(TAPi18n.__('alerts.tooMuch'), TAPi18n.__('alerts.tooMuch2', template.howManyLeft.get()));
        else {
            showConfirmDialog({
...
Template.pagePanelCrm.onCreated(function() {
    ...
    this.howManyEmailsICanSend = new ReactiveVar(Settings.findOne().howManyEmails);
    ...
    this.howManyLeft = new ReactiveVar(parseInt(this.howManyEmailsICanSend.get()) - parseInt(howMany));
});
