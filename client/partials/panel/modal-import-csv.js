import { SessionDataCsvFilename, SessionDataCsvStatusParsed, SessionDataCsvStatusUploaded, SessionDataCsvContents, SessionDataCsvColumns, SessionDataCsvColumnsMapped, SessionDataCsvUploadReport } from "/client/helpers/sessions";
import Papa from 'papaparse';
import { closeModal, showConfirmDialog, showDangerNotification, showModal, showSuccessNotification } from "/client/helpers/helpers";
...
Template.modalImportCsv.helpers({
    csvFilename: function() {
        return SessionDataCsvFilename.get() ? SessionDataCsvFilename.get() : TAPi18n.__('crm.import.choose');
    },
...
Template.modalImportCsv.events({
    ...
    'click #uploadCsvAndParse': function(event, template) {
        Papa.parse(document.querySelector('#csvFile').files[0], {
            complete(results) {
                ...
                SessionDataCsvColumns.set(csvColumns.map(function(column, index) {
                    return {
                        index: index,
                        value: column
                    }
    ...
    'click #mapCsvAndPushToServer': function(event) {
        ...
        const mappedColumnHeadSelects = document.querySelectorAll('[name="csv-columns[]"]');
        ...
        const categoriesCSV = $('#categoriesCSV').val() ? $('#categoriesCSV').val().split(',') : [];
