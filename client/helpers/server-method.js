...
import { errorDebugLog, startedDebugLog, successDebugLog } from "/client/helpers/debug-log";
import { showSuccessAlert, showWarningAlert, showInfoAlert, showSuccessNotification, showInfoNotification, showWarningNotification, showDangerNotification } from "/client/helpers/helpers";
...
export function callServerMethod(args) {
	...
	Meteor.call(args.methodName, args.params, function(error, result) {
		...
		successDebugLog(`Method::stop():\t ${args.methodName} :\t lasts for ${Math.abs(new Date().getTime() - startTime.getTime())}ms`);
